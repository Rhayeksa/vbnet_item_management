...
Public Class CustomerDetail
    Private Sub CustomerDetail_Activated(sender As Object, e As EventArgs) Handles MyBase.Activated
        Call Clean()
    End Sub

    Sub Clean()
        TextBox1.Text = Customer.SelectedRow.Cells(0).Value.ToString()
        Call Connect()
        Cmd = New SqlCommand($"SELECT name FROM customer WHERE id = '{TextBox1.Text}'", Conn)
        TextBox4.Text = IIf(IsDBNull(Cmd.ExecuteScalar), "", Cmd.ExecuteScalar)
        Cmd = New SqlCommand($"SELECT address FROM customer WHERE id = '{TextBox1.Text}'", Conn)
